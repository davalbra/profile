...
async function resolveSourceImage(
  ...
  if (galleryPath) {
    ...
    throw new Error(
      "Debes seleccionar una imagen local, de galería o optimizada.",
    )
  }
  ...
  const fileName =
    originalName || storagePath.split("/").pop() || `image-${Date.now()}`
...
function shouldForceJpegConversion(formData: FormData): boolean {
  const rawValue = String(formData.get("forceJpegConversion") || "")
    .trim()
    .toLowerCase()
...
function shouldPrepareOnly(formData: FormData): boolean {
  const rawValue = String(formData.get("prepareOnly") || "")
    .trim()
    .toLowerCase()
  return rawValue === "1" || rawValue === "true" || rawValue === "yes"
}
...
function shouldOptimizeForWeb(formData: FormData): boolean {
  const rawValue = String(formData.get("optimizeForWeb") || "")
    .trim()
    .toLowerCase()
  return rawValue === "1" || rawValue === "true" || rawValue === "yes"
}
...
function normalizeN8nStoredFileName(
  fileName: string,
  contentType: string,
): string {
  const trimmed = fileName.trim()
  ...
  const withoutExtension =
    safeRawName.replace(/\.[^.]+$/, "").replace(/^-|-$/g, "") ||
    `n8n-image-${Date.now()}`
  const providedExtension = extractFileExtension(safeRawName)
  const validProvidedExtension = mimeFromFileExtension(providedExtension)
    ? providedExtension
    : null
...
async function saveImageToN8nFolder(input: {
  ...
  const normalizedFileName = normalizeN8nStoredFileName(
    input.fileName,
    input.contentType,
  )
...
async function saveOptimizedWebImage(input: {
  ...
  const optimizedBuffer = await sharp(input.sourceBuffer, {
    failOn: "none",
    animated: true,
  })
    .rotate()
  ...
  const sourceExtension =
    extractFileExtension(input.sourceFileName) ||
    extensionFromMime(input.sourceContentType)
  ...
  const savedBytes = Math.max(
    0,
    input.sourceBuffer.length - optimizedBuffer.length,
  )
  const savedPercent =
    input.sourceBuffer.length > 0
      ? Number(((savedBytes / input.sourceBuffer.length) * 100).toFixed(1))
      : 0
...
async function prepareImageForN8n(
  ...
  try {
    converted = await sharp(sourceImage.buffer, {
      failOn: "none",
      animated: true,
    })
      .rotate()
...
export async function POST(request: Request) {
  try {
    ...
    if (
      sourceImage.source === "gallery" &&
      sourceImage.storagePath &&
      preparedImage.wasConvertedToJpeg
    ) {
    ...
    if (!n8nCopyWebhookUrl) {
      return jsonResponse(
        {
          error:
            "Falta configurar N8N_MCP_NANO_BANANA_WURL en variables de entorno.",
        },
        { status: 500 },
      )
    }
    ...
    outbound.append(
      "image",
      new Blob([new Uint8Array(preparedImage.buffer)], {
    ...
    const n8nHeaderContentType = normalizeMime(
      response.headers.get("content-type"),
    )
    const n8nBuffer = Buffer.from(await response.arrayBuffer())
    const n8nImageFileNameFromHeaders = extractFileNameFromContentDisposition(
      n8nContentDisposition,
    )
    const n8nMimeFromFileName = mimeFromFileExtension(
      extractFileExtension(n8nImageFileNameFromHeaders),
    )
    const n8nMimeFromBuffer = sniffImageMimeFromBuffer(n8nBuffer)
    const n8nDetectedImageMime = n8nHeaderContentType.startsWith("image/")
      ? n8nHeaderContentType
      : n8nMimeFromFileName || n8nMimeFromBuffer
    const n8nResponseIsImage = Boolean(n8nDetectedImageMime)
    const n8nContentType =
      n8nDetectedImageMime || n8nHeaderContentType || "application/octet-stream"
    ...
    let storedN8nImage: StoredN8nImage | null = null
    let optimizedWebImage: StoredOptimizedImage | null = null
      ...
      if (response.ok) {
        ...
        if (optimizeForWeb) {
          optimizedWebImage = await saveOptimizedWebImage({
            ...
            sourceStoragePath:
              storedN8nImage?.path || preparedImage.storagePath || null,
          })
        }
      }
    } else {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No se pudo enviar la imagen a n8n."

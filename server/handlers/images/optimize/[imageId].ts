...
async function readStorageSnapshot(path: string): Promise<{
  ...
  const tokenRaw =
    asString(metadata.metadata?.firebaseStorageDownloadTokens) || ""
...
async function buildLineagePaths(input: {
  uid: string
  ...
  while (cursor && depth < 8 && !visited.has(cursor)) {
    ...
    const relation: { origenPath: string } | null =
      await prisma.imagenRelacion.findFirst({
        where: {
          usuarioId: input.uid,
          destinoPath: cursor,
        },
...
export async function GET(
  ...
  try {
    ...
    const sesion = await requerirSesionFirebase(_request, {
    ...
    if (!image) {
      return jsonResponse(
        { error: "No se encontró la imagen optimizada." },
        { status: 404 },
      )
    }
    ...
    let sourceCollection: "gallery" | "n8n" | "optimized" | "local" | null =
      null
    ...
    if (optimizedExists) {
      ...
      const rawCollection = asString(
        optimizedMetadata.metadata?.sourceCollection,
      )
      sourceCollection =
        ...
        rawCollection === "local"
          ? rawCollection
          : null
      sourceStoragePath = asString(
        optimizedMetadata.metadata?.sourceStoragePath,
      )
      sourceWasN8n =
        asBoolean(optimizedMetadata.metadata?.sourceWasN8n) ||
    ...
    for (const path of uniquePaths) {
      if (path === image.pathOptimizada) {
        lineageNodes.push({
          ...
          contentType:
            optimizedSnapshot?.contentType || image.mimeOptimizado || null,
    ...
    const transitions = lineageNodes.slice(1).map((toNode, index) => {
      ...
      const savedPercent =
        fromBytes > 0
          ? Number(((savedBytes / fromBytes) * 100).toFixed(1))
          : null
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No se pudo cargar el detalle de la imagen optimizada."

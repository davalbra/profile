...
function normalizeStoredImageName(rawName: string): string {
  ...
  return sanitized.toLowerCase().endsWith(".avif")
    ? sanitized
    : `${sanitized}.avif`
}
...
function toImageResponse(
  ...
  const savedBytes =
    latestStats?.bytesAhorrados ??
    Math.max(0, image.bytesOriginal - image.bytesOptimizado)
  const savedPercent =
    latestStats?.porcentajeAhorro ??
    (image.bytesOriginal > 0
      ? Number(((savedBytes / image.bytesOriginal) * 100).toFixed(1))
      : 0)
  ...
  return {
    ...
    downloadURL: getDownloadUrl(
      bucketName,
      image.pathOptimizada,
      image.tokenOptimizado,
    ),
...
async function getLatestOptimizationStats(
  imageIds: string[],
): Promise<Map<string, OptimizationStatsRow>> {
  if (!imageIds.length) {
    return new Map()
  }
...
export async function GET(request: Request) {
  try {
    ...
    const statsByImageId = await getLatestOptimizationStats(
      images.map((image) => image.id),
    )
    const items = images.map((image) =>
      toImageResponse(image, statsByImageId, bucketName),
    )

    return jsonResponse(
      { ok: true, images: items },
...
export async function POST(request: Request) {
  try {
    ...
    if (fileValue instanceof File) {
      ...
      const allowedGalleryPrefix = buildFolderPrefix(
        sesion.uid,
        IMAGE_GALLERY_FOLDER,
      )
      const allowedN8nPrefix = buildFolderPrefix(sesion.uid, IMAGE_N8N_FOLDER)
      const allowedOptimizedPrefix = buildFolderPrefix(
        sesion.uid,
        IMAGE_OPTIMIZED_FOLDER,
      )
      ...
      if (!isAllowedGalleryPath) {
        return jsonResponse(
          {
            error:
              "No tienes permisos para usar esta imagen de galería/n8n/optimizadas.",
          },
          { status: 403 },
        )
      }
      ...
      if (!exists) {
        return jsonResponse(
          { error: "La imagen seleccionada no existe." },
          { status: 404 },
        )
      }
      ...
      sourceName =
        originalName || galleryPath.split("/").pop() || `imagen-${Date.now()}`
      sourceMime = metadata.contentType || "application/octet-stream"
      if (sourceCollection === "optimized") {
        sourceWasN8n =
          asBoolean(metadata.metadata?.sourceWasN8n) ||
          asString(metadata.metadata?.sourceCollection) === "n8n"
      }
    } else {
      return jsonResponse(
        {
          error:
            "Debes enviar un archivo en el campo image/file o indicar galleryPath.",
        },
        { status: 400 },
      )
    }
    ...
    const porcentajeAhorro =
      sourceBuffer.length > 0
        ? Number(((bytesAhorrados / sourceBuffer.length) * 100).toFixed(1))
        : 0
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No se pudo optimizar y subir la imagen."
    return jsonResponse({ error: message }, { status: 500 })
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "No se pudo renombrar la imagen."
    return jsonResponse({ error: message }, { status: 500 })
  }
}

...
function parseAuthError(error: unknown) {
  if (
    error instanceof AccesoDenegadoError ||
...
function mapStorageFileToResponse(
  file: FirebaseFileWithMetadata,
  bucketName: string,
): GalleryImageResponse | null {
  const tokenRaw =
    asString(file.metadata.metadata?.firebaseStorageDownloadTokens) || ""
...
async function buildN8nScopedGallery(input: {
  ...
  const compatibleByOrigin = new Map(
    compatibleRelations.map((relation) => [relation.origenPath, relation]),
  )
  const responseByOrigin = new Map(
    responseRelations.map((relation) => [relation.origenPath, relation]),
  )
  const uniqueDerivedPaths = Array.from(
    new Set(
      [...compatibleRelations, ...responseRelations].map(
        (relation) => relation.destinoPath,
      ),
    ),
  )
  ...
  const staleRelationIds = [
    ...staleCompatibleRelationIds,
    ...staleResponseRelationIds,
  ]
  ...
  return input.galleryItems.map((item) => {
    ...
    const compatibleDerived = compatibleRelation
      ? derivedByPath.get(compatibleRelation.destinoPath) || null
      : null
    const currentItem = compatibleDerived || item
    const responseRelation = responseByOrigin.get(currentItem.path)
    const responseDerived = responseRelation
      ? derivedByPath.get(responseRelation.destinoPath) || null
      : null
...
async function buildOptimizedScopedGallery(input: {
  uid: string
  bucketName: string
}): Promise<GalleryImageResponse[]> {
  ...
  const mapped: Array<GalleryImageResponse | null> = await Promise.all(
    rows.map(async (row): Promise<GalleryImageResponse | null> => {
      ...
      const [metadata] = await file.getMetadata()
      const sourceCollection = parseSourceCollection(
        metadata.metadata?.sourceCollection,
      )
      const sourceWasN8n =
        asBoolean(metadata.metadata?.sourceWasN8n) || sourceCollection === "n8n"
      ...
      return {
        ...
        name:
          row.nombreOptimizado ||
          row.pathOptimizada.split("/").pop() ||
          row.pathOptimizada,
...
export async function GET(request: Request) {
  try {
    ...
    if (scope === "optimized") {
      ...
      return jsonResponse(
        { ok: true, scope, images: optimizedItems },
    ...
    const baseItems = fileWithMetadata
      .map((file) =>
        mapStorageFileToResponse(file as FirebaseFileWithMetadata, bucket.name),
      )
...
export async function POST(request: Request) {
  try {
    ...
    if (!(fileValue instanceof File)) {
      return jsonResponse(
        { error: "Debes enviar un archivo en el campo image o file." },
        { status: 400 },
      )
    }
    ...
    const extension = fileValue.name.includes(".")
      ? fileValue.name.split(".").pop()?.toLowerCase()
      : "bin"
    ...
    if (!item) {
      return jsonResponse(
        { error: "No se pudo construir la respuesta de la imagen." },
        { status: 500 },
      )
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No se pudo subir la imagen a galería."
    return jsonResponse({ error: message }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!path) {
      return jsonResponse(
        { error: "Falta path del archivo a eliminar." },
        { status: 400 },
      )
    }
    ...
    const relatedRelations = await prisma.imagenRelacion.findMany({
      where: {
        ...
        OR: [{ origenPath: path }, { destinoPath: path }],
      },
      select: {
        id: true,
        tipo: true,
    ...
    if (n8nDerivedPathsToDelete.length > 0) {
      await Promise.all(
        n8nDerivedPathsToDelete.map((derivedPath) =>
          bucket.file(derivedPath).delete({ ignoreNotFound: true }),
        ),
      )
    }
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!path.startsWith(prefix)) {
      return jsonResponse(
        { error: "No tienes permisos para renombrar este archivo." },
        { status: 403 },
      )
    }
    ...
    const [metadata] = await file.getMetadata()
    const currentCustomMetadata = (metadata.metadata || {}) as Record<
      string,
      string
    >
    ...
    if (!item) {
      return jsonResponse(
        { error: "No se pudo construir la imagen actualizada." },
        { status: 500 },
      )
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No se pudo renombrar la imagen de galería."
    return jsonResponse({ error: message }, { status: 500 })
  }
}

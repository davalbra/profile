...
import { alcanceGaleriaPredeterminado } from "@/utils/constants/imagenes"
import type { AlcanceGaleriaImagen } from "@/utils/enums/anums"

const GALLERY_CACHE_TTL_MS = 5 * 60 * 1000

export type GalleryScope = AlcanceGaleriaImagen
...
function getUserCacheEntry(
  userId: string,
  scope: GalleryScope,
): GalleryCacheEntry | null {
...
async function requestGalleryImages(
  force: boolean,
  userId: string,
  scope: GalleryScope,
  ...
  const request = (async () => {
    const { data } = await imagenesRepositorio.obtenerGaleria(scope)
...
export function invalidateGalleryImagesCache(input?: {
  userId?: string | null
...
export function useGalleryImages(input: {
  ...
  const currentScope = computed(
    () => toValue(input.scope) || alcanceGaleriaPredeterminado,
  )
  const canLoad = computed(
    () => (toValue(input.enabled) ?? true) && Boolean(toValue(input.userId)),
  )

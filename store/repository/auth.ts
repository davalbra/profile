...
import type { RespuestaSesionFirebase, RespuestaSesionSegura } from "@/types/auth";

export const useAuthRepositorio = defineStore("authRepositorio", () => {
  const crearSesionFirebase = async (idToken: string) => {
    return await axios.post<RespuestaSesionFirebase>("/api/auth/firebase-session", { idToken });
  };

...
import { runFuturesBot, type FuturesBotOverrides } from "@/lib/trading/binance-futures";
import { saveRunHistory } from "@/lib/trading/history";
...
function parseOverrides(payload: RequestPayload): FuturesBotOverrides {
  ...
  return {
    ...
    dryRun: parseBoolean(payload.dryRun),
...
export async function POST(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : "No se pudo ejecutar el bot de trading.";
    const status = /binance|config|inv[aá]lid|minim|saldo|balance/i.test(message) ? 400 : 500;

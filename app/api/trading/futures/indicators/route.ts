...
import { getFuturesIndicators, type FuturesBotOverrides } from "@/lib/trading/binance-futures";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await getFuturesIndicators({
      overrides,
      candlesLimit,
    });
    ...
    const message = error instanceof Error ? error.message : "No se pudieron calcular los indicadores.";
    const status = /binance|config|inv[aá]lid|velas|candle|indicador/i.test(message) ? 400 : 500;

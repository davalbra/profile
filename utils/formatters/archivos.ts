...
export function obtenerExtensionPorTipoContenido(
  tipoContenido: string | null,
): string {
  const tipoNormalizado = (tipoContenido || "")
    .toLowerCase()
    .split(";")[0]
    .trim()
...
export function construirNombreDescargaImagen(entrada: {
  nombre: string
  tipoContenido: string | null
}): string {
  ...
  if (!nombreNormalizado)
    return `${nombreArchivoImagenPredeterminado}.${extension}`

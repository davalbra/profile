...
const selectedGalleryNeedsJpegWizard = computed(() => {
  ...
  return !isN8nSupportedImageFormat({
    ...
    fileName:
      selectedGalleryImage.value.name || selectedGalleryImage.value.path,
  })
})

const busy = computed(() => sending.value)
const totalGalleryPages = computed(() =>
  Math.max(1, Math.ceil(galleryImages.value.length / galleryPageSize.value)),
)
...
async function handleSendToN8n() {
  ...
  sending.value = true
  abrirAlertaProceso("Procesando imagen con n8n")
  try {
    const respuesta = await imagenesRepositorio.enviarImagenN8n({
    ...
    const sourceLabel =
      payload.source === OrigenFlujoImagen.GALERIA
        ? "galería"
        : payload.source === OrigenFlujoImagen.N8N
          ? "galería n8n"
          : "galería"
    const convertedLabel = payload.wasConvertedToJpeg
      ? " (convertida a JPG)"
      : ""
    const storedLabel = payload.n8nStoredImage ? " Resultado n8n guardado." : ""
    const compatibleLabel = payload.n8nCompatibleImage
      ? " Copia JPG intermedia guardada."
      : ""
    ...
    toast.success("Imagen procesada con n8n.")
    ...
    if (payload.n8nStoredImage || payload.n8nCompatibleImage) {
      ...
      selectedGalleryPath.value =
        payload.n8nStoredImage?.path || payload.n8nCompatibleImage?.path || null
    }
  } catch (reason) {
    failure.value =
  ...
  <Card>
    <CardHeader class="space-y-3">
      <div>
        ...
        <CardDescription>
          Elige una imagen, pásala por el webhook de n8n y guarda el resultado
          optimizado para web.
        </CardDescription>
      </div>
    ...
    <CardContent class="space-y-5">
      ...
      <div class="space-y-3 rounded-lg border p-4">
        ...
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          <button
            ...
            :class="
              image.path === selectedGalleryPath
        ...
        <div v-if="selectedGalleryImage" class="space-y-1">
          <p class="text-xs text-muted-foreground">
            Seleccionada: {{ selectedGalleryImage.name }}
          </p>
          <p
            v-if="selectedGalleryNeedsJpegWizard"
            class="text-xs text-amber-700"
          >
            Esta imagen se convertirá a JPG automáticamente antes de enviarla a
            n8n.
          </p>
          <p v-else class="text-xs text-muted-foreground">
            Formato compatible con n8n, envío directo.
          </p>
        </div>
        <p v-else class="text-xs text-muted-foreground">
      ...
      <section v-if="responseImage" class="space-y-2 rounded-lg border p-4">
        ...
        <p class="text-xs text-muted-foreground">
          {{ responseImage.fileName }} · {{ responseImage.contentType }} ·
          {{ formatearBytes(responseImage.sizeBytes) }}
        </p>
        <div
          class="aspect-[4/3] w-full max-w-2xl overflow-hidden rounded-lg border bg-muted"
        >
          <img

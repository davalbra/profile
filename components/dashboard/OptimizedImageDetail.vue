...
async function loadDetail() {
  ...
  try {
    const {data: payload} = await imagenesRepositorio.obtenerDetalleOptimizacion(imageId.value);
    data.value = payload;
  } catch (reason) {
...
<template>
  ...
  <div v-else class="space-y-4">
    <div class="flex flex-wrap items-center gap-2">
      ...
      <Badge variant="outline">{{ formatearBytes(data.image.sizeBytes) }}</Badge>
    </div>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {{ formatearBytes(data.image.originalSizeBytes) }} -> {{ formatearBytes(data.image.sizeBytes) }} |
          ahorro {{ formatearBytes(data.image.savedBytes) }} ({{ formatearPorcentaje(data.image.savedPercent) }})
        </CardDescription>
      </CardHeader>
      <CardContent class="space-y-3">
        ...
        <div class="grid gap-2 text-xs text-muted-foreground md:grid-cols-2">
          ...
          <p>Actualizada: {{ formatearFechaMediaConHora(data.image.updatedAt) }}</p>
          <p>
            Formato original:
      ...
      <CardContent class="space-y-3">
        <div class="grid gap-3 md:grid-cols-3">
          <div v-for="(node, index) in data.lineage" :key="node.path" class="space-y-2">
            <article class="overflow-hidden rounded-lg border">
              ...
              <div class="space-y-1 p-2">
                ...
                <div class="flex flex-wrap gap-1">
                  ...
                  <Badge variant="outline" class="text-[10px]">{{ formatearBytes(node.sizeBytes) }}</Badge>
            ...
            <div v-if="index < data.transitions.length"
              ...
              <p>
                Ahorro {{ formatearBytes(data.transitions[index].savedBytes) }}

...
async function loadData(selectedPeriod = period.value) {
  ...
  try {
    ...
    error.value = reason instanceof Error ? reason.message : "No se pudo obtener el costo por uso.";
  } finally {
    loading.value = false;
  }
}
    ...
    <CardContent class="space-y-5">
      ...
      <div class="grid gap-3 md:grid-cols-3">
        ...
        <div class="rounded-lg border p-4">
          ...
          <p class="mt-1 text-xl font-semibold">
            {{ formatearMoneda(data?.averageDailyCost || 0, data?.currency || "USD") }}
          </p>
        </div>
        <div class="rounded-lg border p-4">
      ...
      <div class="rounded-lg border p-4">
        <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <p v-if="firstChartDate && lastChartDate" class="text-xs text-muted-foreground">
            {{ formatearFechaCorta(firstChartDate) }} - {{ formatearFechaCorta(lastChartDate) }}
          </p>
        </div>
        ...
        <div v-if="chartPoints.length" class="h-[260px] w-full">
          <svg viewBox="0 0 100 40" preserveAspectRatio="none" class="size-full overflow-visible rounded-md">
            ...
            <polyline :points="linePoints" fill="none" class="stroke-primary" stroke-width="1.2" stroke-linecap="round"
          ...
          <TableBody>
            ...
            >
              ...
              <TableCell class="text-right">
                {{ formatearNumero(item.usageAmount) }} {{ item.usageUnit || "" }}
              </TableCell>

...
async function fetchRemoteImageInput(imageUrl: string) {
  ...
  const resolvedFileName = inferFileNameFromUrl(
    parseRemoteImageUrl(resolvedUrl),
  )
...
export function registerOptimizeImageTool(server: McpServer) {
  server.registerTool(
    "optimize_image",
    {
      ...
      description:
        "Optimiza una imagen remota por URL a formato AVIF con metadata de ahorro.",
      inputSchema: {
    ...
    async ({
      ...
      try {
        ...
        const savedBytes = Math.max(
          0,
          optimized.original.sizeBytes - optimized.optimized.sizeBytes,
        )
        const savedPercent =
          optimized.original.sizeBytes > 0
            ? Number(
                ((savedBytes / optimized.original.sizeBytes) * 100).toFixed(2),
              )
            : 0

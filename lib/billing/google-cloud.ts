...
const USAGE_DATE_SQL =
  "COALESCE(DATE(usage_end_time), DATE(usage_start_time), DATE(export_time))"
...
function parseBillingExportTable(rawTable: string): string {
  ...
  if (!projectId || !datasetId || !tableId) {
    throw new BillingConfigurationError(
      "GOOGLE_BILLING_EXPORT_TABLE no es válido.",
    )
  }
...
function getBillingConfig(): BillingConfig {
  const rawBillingExportTable = normalizeEnv(
    process.env.GOOGLE_BILLING_EXPORT_TABLE,
  )
...
function getDateRange(period: BillingPeriodKey): {
  startDate: string
  endDate: string
} {
  const now = new Date()
...
async function getGoogleCloudAccessToken(): Promise<string> {
  ...
  if (!credential || typeof credential.getAccessToken !== "function") {
    throw new Error(
      "No se pudo obtener credenciales para consultar Google Cloud Billing.",
    )
  }
...
async function queryBigQuery(input: {
  ...
  const payload = (await response
    .json()
    .catch(() => null)) as BigQueryQueryResponse | null
  ...
  if (!response.ok) {
    const details =
      errorMessage || `BigQuery respondió con estado ${response.status}.`
    const iamHint =
      response.status === 403
    ...
    throw new Error(
      `No se pudo consultar costos de billing: ${details}${reasonHint}.${iamHint}`,
    )
  }

  if (!payload?.jobComplete) {
    throw new Error(
      "La consulta de billing tardó más de lo esperado. Intenta de nuevo.",
    )
  }

  if (payload.errors?.length) {
    throw new Error(
      payload.errors[0]?.message || "No se pudo consultar billing en BigQuery.",
    )
  }
...
function aggregateUsageData(input: {
  ...
  for (const record of input.records) {
    ...
    if (record.usageUnit) {
      usageByUnit.set(
        record.usageUnit,
        (usageByUnit.get(record.usageUnit) || 0) + record.usageAmount,
      )
    }
...
function buildTopServicesDiagnosticQuery(input: {
  billingExportTable: string
}): string {
  return `
        SELECT
...
function mapTopServices(
  rows: BigQueryRow[],
): Array<{ serviceName: string; netCost: number }> {
  return rows
    .map((row) => {
      const cells = row.f || []
    ...
    .filter(
      (item): item is { serviceName: string; netCost: number } => item !== null,
    )
}

...
export class AccesoDenegadoError extends Error {
  constructor(
    message = "Tu correo no está autorizado para usar esta aplicación.",
  ) {
    super(message)
...
export class RolInsuficienteError extends Error {
  constructor(rolActual: RolUsuario, rolMinimo: RolUsuario) {
    super(
      `Se requiere rol ${rolMinimo} para acceder. Tu rol actual es ${rolActual}.`,
    )
...
async function validarCorreoAutorizado(
  email: string,
...
export async function registrarSesionFirebase(
  idToken: string,
  request: Request,
) {
...
export async function validarSesionFirebase(
  token: string,
